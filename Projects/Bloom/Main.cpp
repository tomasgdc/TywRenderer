...
private:
	...
	struct {
		...
		// Text overlay submission andexecution
		VkSemaphore textOverlayComplete;

		//Ofscreen buffer submission and execution
		VkSemaphore ofscreenComplete;
	} Semaphores;
...
private:
	...
	struct FrameBuffers
	{
		VkTools::FrameBuffer frameBuffer;
		VkTools::FrameBufferAttachment depth, colorscene, colorbright;
	};
...
public:
	...
	void BuildOfscreenCommandBuffer();
...
void Renderer::CreateFrameBuffer()
{
	...
	offscreenFramebuffer.frameBuffer.width = GBUFF_DIM;
	offscreenFramebuffer.frameBuffer.height = GBUFF_DIM;


	VkTools::CreateFrameBufferAttachement(
		offscreenFramebuffer.frameBuffer.width,
		offscreenFramebuffer.frameBuffer.height,
		VK_FORMAT_R16G16B16_SFLOAT,
		VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, 
		&offscreenFramebuffer.colorscene,
		layoutCmd,
		m_pWRenderer->m_SwapChain.device,
	...
	VkTools::CreateFrameBufferAttachement(
		...
		VK_FORMAT_R16G16B16_SFLOAT,
		VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
		&offscreenFramebuffer.colorbright,
		layoutCmd,
		m_pWRenderer->m_SwapChain.device,
		m_pWRenderer->m_DeviceMemoryProperties);
	...
	//Deffered FrameBuffer
	{
		...
		// Formats
		attachmentDescs[0].format = offscreenFramebuffer.colorscene.format;
		attachmentDescs[1].format = offscreenFramebuffer.colorbright.format;
		attachmentDescs[2].format = offscreenFramebuffer.depth.format;
		...
		VK_CHECK_RESULT(vkCreateRenderPass(m_pWRenderer->m_SwapChain.device, &renderPassInfo, nullptr, &offscreenFramebuffer.frameBuffer.renderPass));

		std::array<VkImageView, 3> attachments;
		attachments[0] = offscreenFramebuffer.colorscene.view;
		attachments[1] = offscreenFramebuffer.colorbright.view;
		attachments[2] = offscreenFramebuffer.depth.view;

		VkFramebufferCreateInfo fbufCreateInfo = {};
		fbufCreateInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
		fbufCreateInfo.pNext = NULL;
		fbufCreateInfo.renderPass = offscreenFramebuffer.frameBuffer.renderPass;
		fbufCreateInfo.pAttachments = attachments.data();
		fbufCreateInfo.attachmentCount = static_cast<uint32_t>(attachments.size());
		fbufCreateInfo.width = offscreenFramebuffer.frameBuffer.width;
		fbufCreateInfo.height = offscreenFramebuffer.frameBuffer.height;
		fbufCreateInfo.layers = 1;
		VK_CHECK_RESULT(vkCreateFramebuffer(m_pWRenderer->m_SwapChain.device, &fbufCreateInfo, nullptr, &offscreenFramebuffer.frameBuffer.frameBuffer));
	}
...
{
	//Ofscreen Command Buffer
	BuildOfscreenCommandBuffer();

	//Main Command Buffer
	BuildMainCommandBuffer();
}

void Renderer::BuildOfscreenCommandBuffer()
{
	...
	clearValues[1].color = g_DefaultClearColor;
	clearValues[2].depthStencil = { 1.0f, 0 };

	VkRenderPassBeginInfo renderPassBeginInfo = {};
	renderPassBeginInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
	renderPassBeginInfo.pNext = NULL;
	renderPassBeginInfo.renderPass = offscreenFramebuffer.frameBuffer.renderPass;
	renderPassBeginInfo.renderArea.offset.x = 0;
	renderPassBeginInfo.renderArea.offset.y = 0;
	renderPassBeginInfo.renderArea.extent.width = offscreenFramebuffer.frameBuffer.width;
	renderPassBeginInfo.renderArea.extent.height = offscreenFramebuffer.frameBuffer.height;
	renderPassBeginInfo.clearValueCount = 3;
	renderPassBeginInfo.pClearValues = clearValues;


	// Set target frame buffer
	renderPassBeginInfo.framebuffer = offscreenFramebuffer.frameBuffer.frameBuffer;
...
void Renderer::PreparePipeline()
{
	...
	//Main
	{
		...
		shaderStages[0] = LoadShader(GetAssetPath() + "Shaders/Bloom/PostProcess.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = LoadShader(GetAssetPath() + "Shaders/Bloom/PostProcess.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	{
		//Seperate renderpass
		pipelineCreateInfo.renderPass = offscreenFramebuffer.frameBuffer.renderPass;

		//Seperatte layout
		pipelineCreateInfo.layout = offscreenPipelineLayout;

		shaderStages[0] = LoadShader(GetAssetPath() + "Shaders/Bloom/Object.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = LoadShader(GetAssetPath() + "Shaders/Bloom/Object.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void Renderer::StartFrame()
{
...

	{
		...
		//Offscreen
		{
			...
			submitInfo.pSignalSemaphores = &Semaphores.ofscreenComplete;
			VK_CHECK_RESULT(vkQueueSubmit(m_pWRenderer->m_Queue, 1, &submitInfo, VK_NULL_HANDLE));
		}
		...
		//Main
		{
			...
			submitInfo.waitSemaphoreCount = 1;
			submitInfo.pWaitSemaphores = &Semaphores.ofscreenComplete;
